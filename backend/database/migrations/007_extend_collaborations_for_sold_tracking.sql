...
ADD COLUMN IF NOT EXISTS marked_as_sold_at TIMESTAMP WITH TIME ZONE,
  ADD COLUMN IF NOT EXISTS marked_as_sold_by UUID REFERENCES public.users(id),
  ADD COLUMN IF NOT EXISTS confirmed_as_sold_at TIMESTAMP WITH TIME ZONE,
  ADD COLUMN IF NOT EXISTS confirmed_as_sold_by UUID REFERENCES public.users(id);
