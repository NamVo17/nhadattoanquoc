...
interface PropertyFormData {
  ...
  district: string;
  address: string;
   /** Link Google Maps (embed hoặc share URL) */
  mapUrl: string;
  price: number;
  area: number;
  bedrooms: number;
...
export default function PostPropertyPage() {
  ...
  const handleSubmit = async () => {
    // Nếu là gói trả phí → chuyển sang trang thanh toán, chưa tạo tin
    ...
    try {
      ...
      const createdProperty = await propertyService.create(propertyData as any);
      ...
      // Chuyển sang trang chi tiết BĐS sau khi đăng
      if (createdProperty.slug) {
        router.push(`/properties/${createdProperty.slug}`);
      } else {
          ...
          <div className="mb-8 overflow-x-auto">
            <div className="flex border-b border-slate-700 min-w-max">
              <button
                ...
                className={`flex items-center gap-2 pb-4 pr-8 transition-all ${
                  currentStep === 1
              ...
              >
                ...
                <span className="text-sm font-bold">Thông tin cơ bản</span>
              </button>
              <button
                onClick={() => currentStep >= 2 && setCurrentStep(2)}
                className={`flex items-center gap-2 pb-4 px-8 transition-all ${
                  currentStep === 2
                    ? 'border-b-2 border-primary text-primary'
                    : currentStep > 2
                    ? 'border-b-2 border-transparent text-green-400'
              ...
              >
                ...
                <span className="text-sm font-bold">Hình ảnh & Video</span>
              </button>
              <button
                onClick={() => currentStep >= 3 && setCurrentStep(3)}
                className={`flex items-center gap-2 pb-4 pl-8 transition-all ${
                  currentStep === 3
                    ? 'border-b-2 border-primary text-primary'
                    : 'border-b-2 border-transparent text-slate-400'
                }`}
              >
                ...
                <span className="text-sm font-bold">Hoa hồng & Phí dịch vụ</span>
              </button>
            </div>
          </div>
...
function Step1BasicInfo({ formData, updateFormData }: { formData: PropertyFormData; updateFormData: (field: keyof PropertyFormData, value: any) => void }) {
  return (
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              className="w-full rounded-lg border border-slate-400 bg-slate-100 text-slate-600 focus:ring-primary focus:border-primary text-sm px-4 py-3"
              value={formData.type}
              onChange={(e) => updateFormData('type', e.target.value)}
            >
              <option value="" className="bg-slate-100 text-slate-600">Chọn loại hình</option>
              {PROPERTY_TYPES.map(type => (
                <option key={type.value} value={type.value} className="bg-slate-100 text-slate-600">{type.label}</option>
              ))}
            </select>
          </div>
          <div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <select
              className="w-full rounded-lg border border-slate-600 bg-slate-100 text-slate-600 focus:ring-primary focus:border-primary text-sm px-4 py-3"
              value={formData.city}
              onChange={(e) => {
            ...
            >
              <option value="" className="bg-slate-100 text-slate-600">Chọn Tỉnh/Thành</option>
              {CITIES.map(city => (
                <option key={city.value} value={city.value} className="bg-slate-100 text-slate-600">{city.label}</option>
              ))}
            </select>
            <select
              className="w-full rounded-lg border border-slate-600 bg-slate-100 text-slate-600 focus:ring-primary focus:border-primary text-sm px-4 py-3"
            ...
            >
              <option value="" className="bg-slate-100 text-slate-600">Chọn Quận/Huyện</option>
              {formData.city && (DISTRICTS_BY_CITY[formData.city] || []).map(district => (
                <option key={district.value} value={district.value} className="bg-slate-100 text-slate-600">{district.label}</option>
              ))}
            </select>
          </div>
          <input
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-1">
            Bạn có thể dùng link chia sẻ từ Google Maps. Hệ thống sẽ hiển thị bản đồ trong trang chi tiết tin đăng.
          </p>
        </div>
          ...
          <div>
            ...
            >
              {DIRECTION_OPTIONS.map(opt => (
                <option key={opt.value || 'none'} value={opt.value} className="bg-slate-100 text-slate-600">{opt.label}</option>
              ))}
            </select>
          </div>
        </div>
...
// Step 2: Images & Videos
function Step2ImagesVideos({ formData, updateFormData }: { formData: PropertyFormData; updateFormData: (field: keyof PropertyFormData, value: any) => void }) {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Append mới vào danh sách ảnh hiện có
    const newImages = [...formData.images, ...files];
    updateFormData('images', newImages);

    // Tạo preview mới và nối thêm
      ...
      <div className="bg-white rounded-xl p-6 shadow-lg border border-primary/30">
        ...
        <div className="relative border-2 border-dashed border-slate-600 rounded-xl p-10 flex flex-col items-center justify-center bg-white hover:border-primary transition-colors cursor-pointer mb-8">
          ...
          <p className="text-sm text-slate-600 mt-1 text-center">
            Hoặc click để chọn ảnh từ máy tính. <br/>Hỗ trợ JPG, PNG (Tối đa 5MB/ảnh)
          </p>
          <input
            type="file"
            id="imageUpload"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {imagePreviews.map((preview, index) => (
            <div key={index} className="relative group aspect-[4/3] rounded-lg overflow-hidden border-2 border-primary">
              ...
              {index === 0 && (
                <div className="absolute top-2 left-2 bg-primary text-white text-[10px] px-2 py-0.5 rounded font-bold uppercase">
                  Ảnh bìa
                </div>
              )}
              <button
                onClick={() => removeImage(index)}
                className="absolute top-2 right-2 size-7 bg-white/90 dark:bg-slate-800/90 rounded-full flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm"
              >
                <span className="material-symbols-outlined text-sm">delete</span>
              </button>
            </div>
          ))}
          {Array.from({ length: Math.max(0, 4 - imagePreviews.length) }).map((_, index) => (
            <div key={`empty-${index}`} className="aspect-[4/3] rounded-lg border-2 border-dotted border-slate-600 flex items-center justify-center bg-slate-100">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-slate-600 mb-2">
              Link video từ Youtube hoặc TikTok
            </label>
            <div className="relative">
...
// Step 3: Commission & Package
function Step3CommissionPackage({ formData, updateFormData }: { formData: PropertyFormData; updateFormData: (field: keyof PropertyFormData, value: any) => void }) {
  return (
    <div className="space-y-8">
      {/* Commission */}
      <div className="bg-white rounded-xl p-6 shadow-lg border border-primary/30">
        ...
        <p className="text-sm text-slate-600 mb-6">
          Mức hoa hồng bạn sẵn sàng chi trả cho môi giới khác khi họ giới thiệu khách hàng chốt giao dịch thành công.
        </p>
        ...
        <div className="space-y-4">
          <label className="block relative cursor-pointer">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border-2 border-slate-600 rounded-xl peer-checked:border-primary peer-checked:bg-primary/10 transition-all gap-4">
              <div className="flex items-start gap-4">
                <div className="bg-gray-400 p-3 rounded-lg text-slate-600">
                ...
                <div>
                  ...
                  <ul className="mt-2 space-y-1">
                    ...
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Tiếp cận khách hàng cơ bản
                    </li>
                  </ul>
                </div>
              </div>
              <div className="text-left md:text-right">
                <span className="text-lg font-black text-slate-800">Miễn phí</span>
              </div>
            </div>
          </label>
          ...
          <label className="block relative cursor-pointer">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border-2 border-slate-600 rounded-xl peer-checked:border-primary peer-checked:bg-primary/10 transition-all gap-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <ul className="mt-2 space-y-1">
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Tự động đẩy tin lên đầu mỗi ngày
                    </li>
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Hiển thị huy hiệu VIP nổi bật
                    </li>
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
              ...
              <div className="text-left md:text-right">
                <span className="text-xl font-black text-primary">50.000đ</span>
                <span className="block text-xs text-slate-600">/ngày</span>
              </div>
            </div>
          </label>
          ...
          <label className="block relative cursor-pointer">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between p-5 border-2 border-slate-600 rounded-xl peer-checked:border-primary peer-checked:bg-primary/10 transition-all gap-4">
              <div className="flex items-start gap-4">
                <div className="bg-indigo-200 p-3 rounded-lg text-indigo-400">
                ...
                <div>
                  ...
                  <ul className="mt-2 space-y-1">
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Xuất hiện trang chủ chuyên mục
                    </li>
                    <li className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Kích thước ảnh đại diện lớn hơn 2x
                    </li>
                    <li className="flex items-center gap-2 text-xs text-slate-600 ">
                      <span className="material-symbols-outlined text-green-400 text-sm">check_circle</span>
                      Hỗ trợ quảng cáo Facebook & Google
                    </li>
                  </ul>
                </div>
              </div>
              <div className="text-left md:text-right">
                <span className="text-xl font-black text-indigo-400">200.000đ</span>
                <span className="block text-xs text-slate-600">/ngày</span>
              </div>
            </div>
          </label>
    ...
    <div className="bg-white rounded-xl shadow-lg border border-primary/30 overflow-hidden">
      ...
      <div className="p-6">
        ...
        <ul className="space-y-4">
          <li className="flex items-center gap-3 text-sm">
            <span className={`material-symbols-outlined text-xl ${
              currentStep > 1 ? 'text-green-400' : currentStep === 1 ? 'text-primary' : 'text-slate-500'
            }`}>
              {currentStep > 1 ? 'check_circle' : 'radio_button_unchecked'}
            </span>
            <span className={`font-medium ${currentStep > 1 ? 'text-green-400' : currentStep === 1 ? 'text-primary' : 'text-slate-400'}`}>
              Thông tin cơ bản
            </span>
          </li>
          <li className="flex items-center gap-3 text-sm">
            <span className={`material-symbols-outlined text-xl ${
              currentStep > 2 ? 'text-green-400' : currentStep === 2 ? 'text-primary' : 'text-slate-500'
            }`}>
              {currentStep > 2 ? 'check_circle' : 'radio_button_unchecked'}
            </span>
            <span className={`font-medium ${currentStep > 2 ? 'text-green-400' : currentStep === 2 ? 'text-primary' : 'text-slate-400'}`}>
              Hình ảnh & Video
            </span>
          </li>
          <li className="flex items-center gap-3 text-sm">
            ...
            <span className={`font-medium ${currentStep === 3 ? 'text-primary' : 'text-slate-400'}`}>
        ...
        <div className="mt-8 pt-6 border-t border-slate-700">
          ...
          <div className="group relative rounded-lg overflow-hidden border border-slate-600 aspect-[4/3] bg-white">
